...
using static SimpleTriggerCollider.Editor.CustomSettings;
...
namespace SimpleTriggerCollider.Runtime
{
    public class TriggerCollider : MonoBehaviour
    {
        ...
        private void Reset()
        {
            ...
            Debug.Log($"No trigger <color=lime>{nameof(Collider)}</color> component found on <color=cyan>{gameObject.name}</color>. <color=yellow>Attempting to add</color> a default <color=lime>{nameof(Collider)}</color>.", this);
            AutoAddCollider();
        }
        ...
        private void AutoAddCollider()
        {
            ...
            //Check for what kind of Collider to create
        ...
        private bool IsTriggerPresent()
        {
            ...
            {
                Debug.LogError($"<color=lime>{nameof(TriggerCollider)}</color> cannot automatically fix the collider setup on <color=cyan>{gameObject.name}</color>, because <color=red>a {nameof(Collider)} component was found and {nameof(TriggerCollider)} only supports {nameof(Collider)} components</color>. Manually remove the <color=lime>{nameof(Collider)}</color> component and replace it with a <color=lime>{nameof(Collider)}</color>.", this);
                return false;
            }
